...
from app.api.endpoints.stt import stt_audio
...
@router.post(
    ...
    summary="Voice cloning endpoint (STT + TTS)",
    description="Takes audio input, transcribes it with STT, then generates speech in the same voice using TTS. Complete voice conversion pipeline."
)
async def clone_speech(
    input_audio: UploadFile = File(..., description="Audio file to clone (for both STT input and voice prompt)"),
    exaggeration: Optional[float] = Form(None, description="Emotion intensity (0.25-2.0)", ge=0.25, le=2.0),
    cfg_weight: Optional[float] = Form(None, description="Pace control (0.0-1.0)", ge=0.0, le=1.0),
    temperature: Optional[float] = Form(None, description="Sampling temperature (0.05-5.0)", ge=0.05, le=5.0),
):
    """Voice cloning pipeline: Audio → STT (transcribe) → TTS (re-speak in cloned voice) → Cloned Audio"""
    
    # Step 1: Validate input audio (same validation as voice upload)
    validate_audio_file(input_audio)
    
    temp_audio_path = None
    try:
        # Step 1: Save uploaded audio to temporary file
        file_ext = os.path.splitext(input_audio.filename.lower())[1]
        temp_fd, temp_audio_path = tempfile.mkstemp(suffix=file_ext, prefix="clone_audio_")
        file_content = await input_audio.read()
        with os.fdopen(temp_fd, 'wb') as temp_file:
            temp_file.write(file_content)
        
        print(f"📁 Temporary audio saved: {temp_audio_path} ({len(file_content)} bytes)")
        
        # Step 2: Transcribe audio using STT
        print("🔄 Running STT transcription...")
        transcribed_text = await stt_transcribe(temp_audio_path)
        
        if not transcribed_text.strip():
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                detail="No speech detected in input audio"
            )
        
        print(f"📝 Transcribed text: '{transcribed_text}'")
        
        # Step 3: Generate speech using the same audio file as voice prompt
        print("🗣️ Running TTS with voice cloning...")
        buffer = await generate_speech_internal(
            text=transcribed_text,
            voice_sample_path=temp_audio_path,  # Use input audio as voice prompt
            exaggeration=exaggeration,
            cfg_weight=cfg_weight,
            temperature=temperature
        )
        
        # Success - return the cloned audio
        response = StreamingResponse(
            io.BytesIO(buffer.getvalue()),
            media_type="audio/wav",
            headers={"Content-Disposition": f"attachment; filename=cloned_voice_{input_audio.filename}"}
        )
...
async def stt_transcribe(audio_path: str) -> str:
    """Helper function to transcribe audio file using STT"""
    from app.core.stt_model import get_stt_pipeline
    import soundfile as sf
    import numpy as np
    
    # Get the STT pipeline directly
    stt_pipeline = get_stt_pipeline()
    if stt_pipeline is None:
        raise Exception("STT pipeline not ready")
    
    # Load audio data
    try:
        data, samplerate = sf.read(audio_path, dtype='float32')
        print(f"Loaded audio for STT: {len(data)} samples at {samplerate}Hz")
    except Exception as e:
        print(f"Failed to load audio for STT: {e}")
        return ""
...
@router.post(
    ...
    summary="Voice cloning endpoint (STT + TTS)",
    description="Takes audio input, transcribes it with STT, then generates speech in the cloned voice using the same audio as reference. Complete voice conversion pipeline."
)
async def clone_speech(
    input_audio: UploadFile = File(..., description="Audio file to clone (used for both STT input and voice prompt)"),
    exaggeration: Optional[float] = Form(None, description="Emotion intensity (0.25-2.0)", ge=0.25, le=2.0),
    cfg_weight: Optional[float] = Form(None, description="Pace control (0.0-1.0)", ge=0.0, le=1.0),
    temperature: Optional[float] = Form(None, description="Sampling temperature (0.05-5.0)", ge=0.05, le=5.0),
):
    ...
    # Step 1: Validate input audio file
    validate_audio_file(input_audio)
    ...
    try:
        ...
        # Step 2: Transcribe audio using STT
        print("🔄 Running STT transcription...")
        transcribed_text = stt_transcribe(temp_audio_path)
        ...
        if temp_audio_path and os.path.exists(temp_audio_path):
            try:
                os.unlink(temp_audio_path)
                print(f"🗑️ Cleaned up temporary audio file: {temp_audio_path}")
            except Exception as e:
                print(f"⚠️ Warning: Could not delete temp file: {e}")


@router.post(
    "/v1/audio/clone/stream",
    response_class=StreamingResponse,
    responses={
        200: {"content": {"audio/wav": {}}},
        400: {"model": ErrorResponse},
    ...
    summary="Streaming voice cloning (STT + TTS)",
    description="Stream the voice conversion process in real-time. Audio → STT → TTS → Stream cloned audio"
)
async def clone_speech_stream(
    input_audio: UploadFile = File(..., description="Audio file for real-time voice cloning"),
    exaggeration: Optional[float] = Form(None, description="Emotion intensity (0.25-2.0)", ge=0.25, le=2.0),
    cfg_weight: Optional[float] = Form(None, description="Pace control (0.0-1.0)", ge=0.0, le=1.0),
    temperature: Optional[float] = Form(None, description="Sampling temperature (0.05-5.0)", ge=0.05, le=5.0),
):
    """Streaming voice cloning: Audio → Live STT → Live TTS → Live Cloned Audio Stream"""
    ...
    try:
        # Step 1: Save uploaded audio to temporary file
        file_ext = os.path.splitext(input_audio.filename.lower())[1]
        temp_fd, temp_audio_path = tempfile.mkstemp(suffix=file_ext, prefix="clone_stream_")
        file_content = await input_audio.read()
        with os.fdopen(temp_fd, 'wb') as temp_file:
            temp_file.write(file_content)
        
        # Step 2: Transcribe audio using STT
        print("🔄 Running STT transcription for streaming...")
        transcribed_text = stt_transcribe(temp_audio_path)
        ...
        # Step 3: Stream TTS generation using the same audio as voice prompt
        print("🗣️ Streaming TTS with voice cloning...")
        return StreamingResponse(
            generate_speech_streaming(
                text=transcribed_text,
                voice_sample_path=temp_audio_path,  # Use input audio as voice prompt
            ...
            media_type="audio/wav",
            headers={
                "Content-Disposition": f"attachment; filename=stream_cloned_{input_audio.filename}",
                "Transfer-Encoding": "chunked",
                "Cache-Control": "no-cache",
